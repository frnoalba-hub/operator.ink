...
- A route exists in `src/App.jsx` (e.g. `/StreamerAnalytics`) and `npm run build` works locally, but production still shows **404 Page Not Found** for that path.
...
4. Confirm the main script hash in view source **changed** and `/StreamerAnalytics` loads.
...
$js = "https://operator.ink/assets/<PAST_HASH_HERE>.js"
(Invoke-WebRequest $js -UseBasicParsing).Content.Contains("StreamerAnalytics")
```

Last line should be `True` after a successful deploy that includes Streamer Analytics.
